...
(function () {
    ...
    UploadImage.prototype = {
            ...
            uploader.on('all', function (type, files) {
                switch (type) {
                    ...
                    case 'startUpload':
                        ...
                        uploader.option('server',location.href.split("build")[0]+url);//lvyu
        ...
        getInsertList: function () {
            var i, data, list = [],
                align = getAlign(),
                prefix = location.origin+"/"+location.pathname.split("/")[1]; //location.href.replace("/index.html","");
            for (i = 0; i < this.imageList.length; i++) {
                data = this.imageList[i];//lvyu
                list.push({
                    src: location.origin+data.url,
                    _src:location.origin+data.url,
                    title: data.title,
                    alt: data.original,
                    floatStyle: align
                });
