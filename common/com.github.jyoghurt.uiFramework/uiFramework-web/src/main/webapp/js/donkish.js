...
(function ($, window,template ,require,undefined) {
    ...
    $.ajax = function (opt) {
        ...
        var _opt = $.extend(opt, {
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                //错误方法增强处理
                window.hide_loading_bar && hide_loading_bar();
                this.showLoadingBar && DF.toggleMainViewMask(false);
//                alert2('系统未知错误，请联系管理员！');
            ...
            beforeSend: function (XMLHttpRequest) {
                if($.ajaxSettings._opt && $.ajaxSettings._opt.beforeSend){
                    $.ajaxSettings._opt.beforeSend(XMLHttpRequest,opt);
                }
            },
            complete: function (XMLHttpRequest, TS) {
                //请求完成后回调函数 (请求成功或失败之后均调用)。
                if($.ajaxSettings._opt && $.ajaxSettings._opt.complete){
                    $.ajaxSettings._opt.complete(XMLHttpRequest,opt);
                }
            }
        });
        //_opt.cache = false;
    ...
    var DonkishFramework = function ($) {
        ...
        this.isInVersionCode = function (versionCode) {
            if(versionCode){
                if($.isArray(vc)){
                    var flag = false;
                    for(var i=0;i<vc.length;i++){
                        if(vc[i]['switchGroupKey'] == versionCode){
                            flag = true;
                            break;
                        }
                    }
        ...
        this.renderPage = function (dom, container, fn, fadeInOut) {
            // console.log('進入開始渲染頁面主方法');
            // console.log('頁面內容：',dom);
            // console.log('頁面容器：',container);
            //TODO  多数据的情况
            if (dom && container) {
                ...
                div.fadeOut('fast', function () {
                    ...
                    function cp() {
                        ...
                        if(forms.length){
                            self.initCustomFormComponent(forms);
                            self.initCustomFormPlaceHolder(forms);
                        }
                        //本身就是FORM情况
                        if((el[0].tagName == 'FORM')){
                            self.initCustomFormComponent(el);
                            self.initCustomFormPlaceHolder(el);
                        }
                        self.initHelp();
                        //debug 全局对象
        ...
        //页面切换的时候 清空一些全局对象 或者 无用dom
        function _clearPage(div) {
            if(div.id=="main-content-view"){
                //清理注释
                div.innerHTML = '';
                //清除 ayncSetF
                ayncSetF = {};
                ...
                $('input[name=file]').each(function(){
                    if(this.style.cursor == 'pointer' && this.style.fontSize=='480px'){
        ...
        //页面切换
        var if_param_back_url = false;
        this.getIfParamBack = function () {
            return if_param_back_url;
        }
        this.gotoPage = function (url, param, fn) {
            console.log('跳转当前url:'+url);
            // console.log('isMenu:'+isMenu);
            var _url = '/Welcome',self = this;
            ...
            if (!url) {
                ...
                if(url == '-1' ){
                    ...
                    history.go(-1);
                    console.log('urlParam_back');
                    if_param_back_url = true;
                    return;
                }else if(url=='-2'){
                    //TODO 缓存的视图
                }else{
            ...
            if(param){
                var pageParam = encodeURIComponent(JSON.stringify(param));
                if(url.indexOf('\?')==-1){
                    url = url + '?pageParam=' +  pageParam;
                }else{
                    url = url + '&pageParam='+ pageParam;
                }
            }
        ...
        //url 地址 param传递参数 event hashchange的事件
        this._changePage = function (url,param,event,fn) {
            var self = this,tar = 'main-content-view';
            console.log('加载当前url:'+ url);
            self.getPage(url, function (page) {
                ...
                _urlParam = {};
                //跳转页面的时候保存查询参数
                //如果往回跳转 不保存 url == lastPage
                if(url===lastPage && if_param_back_url){
                    //返回上一页
                    console.log(url);
                    console.log('oldeUrl:'+(event.oldURL || event.originalEvent.oldURL));
                }else{
                    if($('.senior_wrapper').length && url !== currentPage){
                ...
                if(self.event.beforePageSwitch(document.location.hash,function (flag) {
                        if(flag){
                            console.log('回调加载...',url)
                            __renderPage();
                        }
                    })){
                    console.log('正常加载...',url)
                    __renderPage();
                }else{
                    console.log('等待回调...',url)
                }

                function __renderPage() {
                    _mainViewIsOverlay(false);
                    console.log('before render ...',url);
                    self.renderPage(page, tar, function () {
                        lastPage = currentPage;
                        currentPage = url;
                        if_param_back_url = false;
                        self.event.afterPageSwitch();
                        fn && fn()
                        console.log('框架处理页面完成...',url);
                    });
                }
            });
        }
        ...
        //判断是否为input select textarea
        function _isValueableElement(element) {
            if(element instanceof jQuery){
                return element.is('input') || element.is('select') || element.is('textarea');
            }else {
                return false;
            }
        }
                ...
                function _initSingleForm ($form){
                    ...
                    elements.each(function () {
                        ...
                        if (!$this.hasClass('no-placeholder')) {
                            ...
                            if(!$this.hasClass('uEditor') && !$this.hasClass('select2-container') && !$this.is('textarea') ){
                                $this.css({'padding-right': paddingRight + 'px'});
                            }
                            if($this.hasClass('select2-container') && !$this.hasClass('select2-container-disabled')){
                                $this.find('a.select2-choice').css({'padding-right': paddingRight + 'px'});
                            }
                            var placeholder = $('<span class="DF-placeHolder pleft">' + label + '</span>');
                            if(_isValueableElement($this)){
                                //必填项* redstar
                                ...
                                if($this.hasClass('select2-container')){
                                    if(redPoint && $this.next().attr('required')=='required'){
                                        placeholder = $('<span class="DF-placeHolder pleft">'+ '<span class="text-danger" style="vertical-align: middle;font-size: 15px;font-weight: bold;">*</span>'+ label +'</span>');
                                    }
                                }
                            }
                            ...
                            if($this.is('input') || $this.is('textarea')){
                                ...
                                placeholder.css('cursor','pointer').on('click',function () {
                                    $('a.select2-choice',$(this).parent()).trigger('mousedown');
                                });
                            }
                            //如果有默认的值
        ...
        this.showModal = function (config, fn, type) {
            ...
            if (df.dom) {
                ...
                if (!df.cache || dom.selector != lazyLoadFormId) {
                    ...
                    element.off('hide.bs.modal').on('hide.bs.modal',function(event){
                        ...
                        //去掉 main-content-view-loading
                        $('.main-content-view-loading',element).remove();
                    });
                }
                //非组件情况
                if (!type) {
            ...
            if (config.btns && config.btns.length) {
                ...
                for (var i = 0; i < items.length; i++) {
                    var cfg = items[i];
                    //版本
                    if(!cfg.versionCode || (vc && _isVersionCodePassByVersionControlData(vc,cfg.versionCode))){
                        //权限
            ...
            var modal = element.modal({keyboard:false});
            //暴漏获取form的方法
        ...
        $.fn.setReadOnly = function (flag) {
            ...
            if ($this.hasClass('iswitch') && $this.attr('type') == 'checkbox') {
                if(flag){
                    $this.hide();
                    $this.next().show().prop('readonly','readonly');
                    // $this.parent().append('<span class="DF-placeHolder pleft pright">' + $this.parent().find('label').text() + ($this.val()?'是':'否')+ '</span>');
                    $this.parent().append('<span class="DF-placeHolder pleft pright">' + $this.parent().find('label').text() +  '</span>');
            ...
            //遍历form下的所有组件 置为只读 除了ckeditor
                ...
                if(flag){
                    ...
                    $('.DF-placeHolder',$this.parent()).css('cursor','initial');
        ...
        this.clearAyncSetF = function (obj) {
            if(!obj){
                obj = {};
            }
            ayncSetF = obj;
        }
        this.setAyncSetF = function (obj) {
            if($.isPlainObject(obj)){
                ayncSetF = $.extend(ayncSetF,obj);
            }
        }
        ...
        function renderCombobox($this, multiple) {
            ...
            if (multiple) {
                ...
                $this.val(value).select2({
                    ...
                    var $select = $(this);
                    $select.data('select2').results.addClass('overflow-hidden').perfectScrollbar();
                    var $option = $('option[deleteflag=true]',$select);
                    var texts = [];
                    $option.each(function () {
                        texts.push($(this).text());
                    })
                    $('#select2-drop li .select2-result-label').each(function () {
                        var text = $(this).text().trim();
                        if(texts.indexOf(text) !==-1){
                            if($select.attr('showDelete')=="false"){
                                $(this).remove();
                            }else{
                    ...
                    var texts = [];
                    if(select.value){
                        $(select).val().map(function (v) {
                            var $option = $('option[value='+v+']',select);
                            if($option.attr('deleteFlag') === "true" ){
                                texts.push($option.text());
                            }
                        });
                        $('.select2-search-choice',$(select).prev()).each(function () {
                ...
                if($this.attr('searchable')==="true"){
                    $this.val(value).select2({
                        ...
                        // var option = event.added && event.added.element[0];
                        // if(option){
                        //     if($(option).attr('deleteFlag')=="true"){
                        //         $('#select2-chosen-2').css('text-decoration','line-through');
                        //     }else{
                        //         $('#select2-chosen-2').css('text-decoration','none');
                        //     }
                        // }

                        var select = this;
                        if(select.value){
                            var $option = $('option[value='+select.value+']',select);
                            if($option.attr('deleteFlag') === "true" ){
                                $('#select2-chosen-2').css('text-decoration','line-through');
                            }else{
                                $('#select2-chosen-2').css('text-decoration','none');
                    ...
                    $this.val(value).selectBoxIt(config).on('open', function () {
                        ...
                        if($this.attr('selectTree')){
                            var id = $this.attr('id');
                            if(!id){
                                console.error('下拉树必须指定一个ID')
                                return
                            }
                            var treeId = id+'_select_tree';
                            var selectContainer = $this.next()
                            ...
                            if($('#'+treeId).length===0){
                                var ztreeSelect = $('<div class="selectTree ztree" id="'+treeId+'"></div>');
                                ul.find('li').first().before(ztreeSelect);
                                $this.attr('treeId',treeId);
        ...
        this.dataDictTree = function(cfg){
                        ...
                        if(df.selecttree){
                            if((!df.selectLevel || df.selectLevel == treeNode.level) && treeNode.id && treeNode.text){
                                ...
                                if(df.cascadeParentName){
                                    ...
                                    selectContainer.find('.selectboxit-text').text(text);
                                }else{
                                    selectContainer.find('.selectboxit-text').text(treeNode.text);
                                }
                                var v = treeNode.dictValueCode || treeNode[df.idKey];
                                element.find('option').first().val(v);
                                element.val(v);
                                element.prev().val(treeNode.dictItemCode);
                                selectContainer.find('.selectboxit-arrow-container').trigger('click');
                                element.attr('df-change') && setTimeout(function () {
                                    element.trigger('DF.df-change');
                                },0)
            ...
            var selectContainer = element.next(),
                treeObj,
                treeContainer = element.attr('treeid');
            //如果是数据字典
            if(df.dictItemCode){
        ...
        function parseData($this, data, group) {
            ...
            for (var k in data) {
                ...
                op.attr('deleteFlag', data[k]['deleteFlag']);
                if (group) {
                    group.append(op);
                } else {
                    $this.append(op);
        ...
        this.dataTable = function (config) {
            ...
            //是否隐藏操作列
                hideOptionCol = false,
            //是否显示分页信息
                pageDisplay = config.pageDisplay !== false,
            //第一次加载列表的url
                initUrl = '',
            // 总记录数
                recordsTotal = 0;
            ...
            if (rowbtns.length) {
                ...
                columnDefs.push({
                    render: function (data, type, row) {
                        ...
                        for (var i = 0; i < rowbtns.length; i++) {
                            var btn = rowbtns[i];
                            //版本 1.如果页面没有版本管理按钮 2.或者配置了版本按钮 有后台配置 且后台配置包含版本按钮
                            if(!btn.versionCode || (vc && _isVersionCodePassByVersionControlData(vc,btn.versionCode))){
                ...
                if(hyperlinkBtn){
                    columnDefs.push({
                        render:function (data, type, row) {
                            var text = hyperlinkBtn.text || data;
                            var name = hyperlinkBtn.name || 'view';
                            var a = $('<a>');
                            a.attr({
                                href: 'javascript:;',
                                name: name
                            ...
                            a.text(text);
                            hyperlinkBtn.class && a.addClass(hyperlinkBtn.class);
                            hyperlinkBtn.css && a.css(hyperlinkBtn.css);
                            return a[0].outerHTML;
                        },
                        targets:hyperlinkBtn.renderIndex
                    })
                }
            }
            //todo
            if(col.length){
                for(var d=0;d<col.length;d++){
                    ...
                    if(column['editable']){
                        (function(c,i){
                            var renderFn;
                            if(config.columnDefs && config.columnDefs.length){
             ...
             * @returns {*}
             */
            function replaceLeftSpace(url) {
                var o_url = url.substr(0,url.indexOf('\?'));
                var obj = DF.analyticalUrlParams(url);
                for(var k in obj){
                    obj[k] = decodeURIComponent(obj[k]).trimLeft().trimRight();
                }
                url = o_url + '?'+DF.analyticalUrlParams(obj);
                return url
            }
            /**
             *
            ...
            function getSearchUrl(url,seniorSearch,dateElementNames,$form,joinColumn) {
                if(seniorSearch){
                    ...
                    //获取保存的查询条件
                    var search_str = $('#'+tid+'_search_input').val(),visibleColumns = [],s_url;
                    if(senior_param && senior_param['_save_status']&&location.hash == ('#'+senior_param._url_)){
                        search_str = senior_param['_save_status'];
                    }
                    if (!search_str) {
                        return url
                    }else{
            ...
            function retrieveData(sSource, aoData, fnCallback, param) {
                ...
                if(!$('#'+tid).prev().hasClass('search_loaded')){
                    ...
                    //增加loading元素
                    !topwrapper.hasClass('search_loaded') && topwrapper.append('<div id="'+tid+'_processing" class="dataTables_processing" style="">Loading...</div>');
                    //渲染高级搜索
                    if(seniorSearch){
                ...
                if(config.autoParam){
                    var param ;
                    if($.isPlainObject(config.autoParam)){
                        param = config.autoParam;
                    }else if($.isFunction(config.autoParam)){
                        param = config.autoParam();
                    }
                    if($.isPlainObject(param)){
                        url = url + '&' + self.analyticalUrlParams(param);
                    };
                }
                //控制隐藏 加载样式
                var inspect = {
                    beforeFn:function () {
                        ...
                        /**
                         * begin @For Test 清空分页信息
                         * @type {{recordsTotal: *, pageLength: Number, pageNum: (*|jQuery)}}
                         */
                        $('#'+config.id).removeAttr('pageInfo');
                        /**
                         * end
                         */
                    },
                    afterFn:function (data) {
                        ...
                        /**
                         * begin @For Test 添加分页信息
                         * @type {{recordsTotal: *, pageLength: Number, pageNum: (*|jQuery)}}
                         */
                        if(data.result){
                            var pageInfo = {
                                recordsTotal :data.result.recordsTotal,
                                pageLength : data.result.data.length,
                                pageNum : $('#'+config.id).parent().find('input[name=jumper]').val()|| null
                            }
                            $('#'+config.id).attr('pageInfo',JSON.stringify(pageInfo));
                        }
                        /**
                         * end
                         */
            ...
            var df = {
                ...
                bDeferRender: true,
                bSortClasses:false,
                sDom: pageDisplay?"<'row topBar'>t<'row'<'col-xs-6'il><'col-xs-6'p>>":"<'row topBar'>t<'row'<'col-xs-6 hidden'il><'col-xs-6 hidden'p>>",
                ///整个列表加载完后的回调
                language: {
                    "sProcessing": "处理中...",
                    ...
                    (function(btn){
                        ...
                        $('td a[name=' + name + ']', row).click(function () {
                            if(name.startsWith('goto')){
                                ...
                                pageNumber.tid = tid;
                                pageNumber.pageid = self.getCurrentPage();
                            }
            ...
            //隐藏tbody
            $('#' + tid + ' tbody').hide();
            if(typeof pageNumber.number =='number' && (tid ==pageNumber.tid) &&(self.getCurrentPage() ==pageNumber.pageid)){
            ...
            grid = $('#' + tid).DataTable(df).on('draw', function (tb, obj) {
                ...
                var jumpTmpl = '<div id="'+tid+'_page_jumper" class="jump_wrapper"> <input type="number" class="form-control" name="jumper" value="'+n+'"> <button class="btn btn-gray" type="button">GO</button></div>';
        ...
        this.initApp = function (devm) {
            ...
            if (_FLAG) {
                ...
                //版本控制
                if (window.__getVersionCodes) {
                    vc = __getVersionCodes();
                }
                //给开发模式赋值
        ...
        function _resolveTmpl(domStr,data){
            ...
            return _spliceHtmlContent(content);
        }
        /**
         * 组件封装 modal 窗口形式
         */
        this.createWindow = function (param, fn) {
            ...
            if (typeof param == 'object') {
                componentId = param.id + uuid,
                    config.dom = $('#'+param.id),
                    config.data = param.codes || {},
                    config.data['id'] = componentId,
                    config = $.extend(config,param);
            } else if (typeof param == 'string') {
                componentId = param;
            }
            //判断组件是否已经被使用 最多支持3层组件弹出
            if($('#_LYModal_Component').hasClass('in')){
                if($('#_LYModal_Component_Reserve').hasClass('in')){
                    if($('#_LYModal_Component_Reserve2').hasClass('in')){
                        console.error('组件弹出最多支持3层');
                        return
                    }
                    return this.showModal(config, null, '_LYModal_Component_Reserve2');
                }else{
        ...
        this.create = function (param, target, fn) {
            ...
            if (typeof param == 'object' && param['id']) {
                ...
                //var rules = $('#' + componentId).attr('customRules');
            } else if (typeof param == 'string') {
                componentId = param;
                uuid = componentId+uuid;
            ...
            result.html && _bindDFEvents(result.html,eventQueue);
            setTimeout(function () {
                fn && fn(tar, self.getComponent(uuid));
            },0);
            return uuid;
        }
        /**
         ...
         * @private
         */
        function _spliceHtmlContent(content) {
            var html,script;
            if (content.length == 1) {
                html = content.first();
         ...
         * @private
         */
        function _bindDFEvents(html,eventQueue){
            //onchange事件
            ...
            if(OnChangeTags.length){
                OnChangeTags.each(function(){
                    ...
                    $this.bind('DF.df-change',function(e,flag){
                        ...
                        attr && fn && (result = fn.call($this,e,val));
            ...
            if(onClickTags.length){
                onClickTags.each(function(){
                    ...
                    $this.bind('DF.df-click',function(e){
                        ...
                        attr && fn && fn.call($this,e,$this.val());
                    });
                ...
                return {
                    ...
                    nextPage:function (url,callback) {
                        ...
                        return this.init(url,{
                            currentpage:parseInt(store.currentpage) + 1,
        ...
        /**
         * 通过versionCode 判断是否通过版本控制过滤
         * @param ad 数据权限资源
         * @param authCode 权限码
         * @private
         */
        function _isVersionCodePassByVersionControlData(vc,versionCode) {
            var flag = false;
            if(vc && versionCode){
                for(var i=0;i<vc.length;i++){
                    var vcode = vc[i];
                    if(vcode['switchGroupKey'] === versionCode && vcode['switchStatus'] ===true){
                        flag = true;
                        break;
                    }
                }
        ...
        function _mainViewIsOverlay(flag) {
            ...
            if(flag){
                $('.main-content-view-loading').last().removeClass('loaded');
            }else{
        ...
        this.delay = function (fn,time) {
            if(arguments.length==0){
                console.warn('至少一个参数');
                return
            }
            if(typeof fn !=='function'){
                console.warn('请传入一个函数');
                return
            }
            var defaults = {
                fn:function(){},
            ...
            defaults = $.extend(defaults,{fn:fn,time:time});
            timeoutHandle = setTimeout(function () {
                defaults.fn();
                clearTimeout(timeoutHandle);
            },defaults.time);
        }
        this.clearUrlParam = function () {
            urlParam = {};
        }
    };
    ...
    $(window).on('hashchange',function(event){
        ...
        var orgiUrl = document.location.hash ;
        var paramAll = DF.analyticalUrlParams(orgiUrl)||{};
        var  url = orgiUrl.substring(0, orgiUrl.indexOf("?")) || orgiUrl,url = url.replace('\#','');
        var param = {};
        if(paramAll.hasOwnProperty('pageParam')){
            try{
                param = JSON.parse(decodeURIComponent(paramAll['pageParam']));
                for(var k in paramAll){
                    if(k!=='pageParam'){
                        param[k] = paramAll[k];
                    }
                }
            }catch (e){
                console.error("url中传递参数不合法");
                console.error(e.message);
                console.error(e.stack);
            }
        }else{
            param = paramAll;
            var back_param = DF.getBackParam()[url];
            param = $.extend(param,back_param);
        }
        console.log('currentUlr:'+ url,'urlParam:',param);
        DF._changePage(url,param,event,function () {
            //DF.clearUrlParam();
        });
    });
